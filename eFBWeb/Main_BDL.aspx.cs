...
namespace eFBWeb
{
    public partial class Main_BDL : System.Web.UI.Page
    {
        ...
        {
            using (SqlDataAdapter da = new SqlDataAdapter("EXEC PGetListMatch_BDL " + Hour + "," + (chkShowAll.Checked ? "1" : "0") + "", DBConn))
            {
        ...
        {
            if (e.DataColumn.FieldName.Equals("tx_gap"))
            {
            ...
            else if (e.DataColumn.FieldName.Equals("hdc_gap"))
            {
                if (e.CellValue.ToDouble().HasValue)
                {
                    if (Math.Abs(e.CellValue.ToDouble().Value) > 0)
                    {
                        e.Cell.BackColor = System.Drawing.ColorTranslator.FromHtml("#ccffee");
                    }
                }
            }
            else if (e.DataColumn.FieldName.Equals("tx_now_gap"))
            {
                if (e.CellValue.ToDouble().HasValue)
                {
                    if (e.CellValue.ToDouble().Value >= 0.5)
                    {
                        e.Cell.BackColor = System.Drawing.ColorTranslator.FromHtml("#ccffee");
                    }
                }
            }
            else if (e.DataColumn.FieldName.Equals("corner_gap"))
            {
                if (e.CellValue.ToDouble().HasValue)
                {
                    if (e.CellValue.ToDouble().Value >= 3)
                    {
                        e.Cell.BackColor = System.Drawing.ColorTranslator.FromHtml("#ccffee");
                    }
                }
            }
            else if (e.DataColumn.FieldName.StartsWith("h2h") && e.DataColumn.FieldName.Contains("corner"))
            {
                double maxCorner = e.DataColumn.FieldName.Contains("h1") ? 2.5 : 5;
                if (string.IsNullOrEmpty(e.CellValue.ToString()) == false)
                {
                    double? val = e.CellValue.ToString().Split(' ')[0].ToDouble();
                    if (val.HasValue)
                    {
                        if (val.Value >= maxCorner)
                        {
                            e.Cell.BackColor = System.Drawing.ColorTranslator.FromHtml("#ccffee");
                        }
                        else if (val.Value < maxCorner)
                        {
                            e.Cell.BackColor = System.Drawing.ColorTranslator.FromHtml("#e6c3ce");
                        }
                    }
                }
            }
